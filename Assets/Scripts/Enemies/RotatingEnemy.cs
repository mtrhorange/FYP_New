...
public class RotatingEnemy : Enemy
{
    ...
    protected override void Move()
    {
        ...
        if (attackTimer <= 0 )
        {
            float angle1 = 50f, angle2 = 50f;

            if (GameManager.instance.player1)
            angle1 = Vector2.Angle(GameManager.instance.player1.transform.position, transform.up);
            if (GameManager.instance.player2)
            angle2 = Vector2.Angle(GameManager.instance.player2.transform.position, transform.up);
    ...
    {
        AIManager.instance.rotatingEnemies.Remove(this.gameObject);
    }
}
